...
static int test_tls_reqd_no_sans_header(const struct TEST_CASE *tp)
{
    ...
    /*
     * Per-site "none" historically resolved to NONE here. With unconditional
     * TLS the policy resolver promotes NONE/MAY to ENCRYPT.
     */
...
static int test_tls_reqd_no_sans_wrappermode(const struct TEST_CASE *tp)
{
    ...
    var_tls_required_enable = 1;
    /*
     * STATE_TLS_NOT_REQUIRED used to cap at MAY in non-wrappermode. With
     * unconditional TLS, MAY is promoted to ENCRYPT: the operator's
     * mandatory-TLS rule wins over the sender's "TLS-Required: no" header.
     */
    want_level = TLS_LEV_ENCRYPT;
 ...
 /*
  * Cleartext-prevention tests. These verify that the SMTP client unconditionally
  * promotes any sub-ENCRYPT level to ENCRYPT, regardless of how the level was
  * reached: global setting, per-site policy, or REQUIRETLS sender header.
  * TLS is mandatory; there is no opt-out.
  */
...
/* Global "none" -> ENCRYPT (operator cannot opt out of TLS). */
...
/* Global "may" (opportunistic) -> ENCRYPT. */
...
/* Per-site "none" -> ENCRYPT (per-site policy cannot opt out of TLS). */
...
/*
 * A sender-supplied "TLS-Required: no" header (RFC 8689) would normally
 * downgrade SECURE to MAY in non-wrappermode. With unconditional TLS the
 * resolver promotes that downgrade to ENCRYPT. The header cannot disable
 * TLS.
 */
...
/* Stronger levels (secure) are not weakened. */
